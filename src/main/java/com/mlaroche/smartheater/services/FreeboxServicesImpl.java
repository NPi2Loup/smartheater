...
public class FreeboxServicesImpl implements FreeboxServices {

	private static final String WS_LOGIN = "http://mafreebox.freebox.fr/api/v8/login/";
	private static final String WS_LOGOUT = "http://mafreebox.freebox.fr/api/v8/login/logout/";
	private static final String WS_SYSTEM_INFO = "http://mafreebox.freebox.fr/api/v8/system/";
	private static final String WS_SYSTEM_DATA = "http://mafreebox.freebox.fr/api/v8/connection/full/";
	private static final String WS_WIFI_DATA = "http://mafreebox.freebox.fr/api/v8/wifi/ap/";
	private static final String WS_ETHERNET_PORT = "http://mafreebox.freebox.fr/api/v8/switch/status/";
	private static final String WS_ETHERNET_CONNECTED_DEVICES = "http://mafreebox.freebox.fr/api/v8/lan/browser/pub/";
	private static final String WS_ETHERNET_DATA_PREFIX = "http://mafreebox.freebox.fr/api/v8/switch/port/";
	...
	public void updateFreebox(final String measureName) {
		...
		try {
			...
			final Map<String, Integer> temps = new HashMap<>();
			final Map<String, Integer> fans = new HashMap<>();
			final FreeboxInfo freeboxInfo = retrieveData(sessionId, temps, fans);

			putInfosToInflux(freeboxInfo, temps, fans, measureName);
	...
	private void retrieveEthernet(final String measureName, final String sessionId) {
		...
		final String ethernetMesureName = measureName + ".ethernet";
		final JsonObject interfaceResponse = callRestWS(WS_ETHERNET_CONNECTED_DEVICES, sessionId, JsonObject.class);
		final List<String> macs = new ArrayList<>();
		final List<String> hostnames = new ArrayList<>();
		if (interfaceResponse.get("success").getAsBoolean()) {
			for (final JsonElement interfaceElt : interfaceResponse.getAsJsonArray("result")) {
				final JsonObject interfaceObj = interfaceElt.getAsJsonObject();
				if (interfaceObj.get("active").getAsBoolean()) {
					final JsonObject access_point = interfaceObj.getAsJsonObject("access_point");
					if (access_point != null && "ethernet".equals(access_point.get("connectivity_type").getAsString())) {
						macs.add(interfaceObj.getAsJsonObject("l2ident").get("id").getAsString());
						hostnames.add(interfaceObj.get("primary_name").getAsString());
					}
				}
			}
		}
		final JsonObject ethernetResponse = callRestWS(WS_ETHERNET_PORT, sessionId, JsonObject.class);
		if (ethernetResponse.get("success").getAsBoolean()) {
			for (final JsonElement ethernetElt : ethernetResponse.getAsJsonArray("result")) {
				...
				final int id = ethernet.get("id").getAsInt();
				final String name = ethernet.get("name").getAsString();
				final String mode = ethernet.get("mode").getAsString();
				final String link = ethernet.get("link").getAsString();
				if ("up".equals(link)) {
	...
	private void retrieveWifi(final String measureName, final String sessionId) {
		...
		final String wifiMesureName = measureName + ".wifi";
		final JsonObject wifiStationResponse = callRestWS(WS_WIFI_DATA, sessionId, JsonObject.class);
		if (wifiStationResponse.get("success").getAsBoolean()) {
			for (final JsonElement wifiStationElt : wifiStationResponse.getAsJsonArray("result")) {
				...
				final int id = wifiStation.get("id").getAsInt();
				final String name = wifiStation.get("name").getAsString();
				updateWifi(id, name, wifiMesureName, sessionId);
			}
		}
	}

	private void updateEthernet(final long ethId, final String ethName, final String ethMode, final String measureName, final List<String> hostnames, final List<String> macs, final String sessionId) {
		...
		final JsonObject ethResponse = callRestWS(WS_ETHERNET_DATA_PREFIX + ethId + "/stats", sessionId, JsonObject.class);
		if (ethResponse.get("success").getAsBoolean()) {
			final JsonObject eth = ethResponse.getAsJsonObject("result");
			final long rx_good_bytes = eth.get("rx_good_bytes").getAsLong();
			final int rx_bytes_rate = eth.get("rx_bytes_rate").getAsInt();
			final long tx_bytes = eth.get("tx_bytes").getAsLong();
			final int tx_bytes_rate = eth.get("tx_bytes_rate").getAsInt();

			final MeasureBuilder statusMeasureBuilder = Measure.builder(measureName)
					.time(Instant.now())
					.addField("ethName", ethName)
					.addField("ethId", ethId)
					.addField("ethMode", ethMode)
					.addField("hostnames", String.join(";", hostnames))
					.addField("hostCount", hostnames.size())
			...
			for (int i = 0; i < hostnames.size(); i++) {
				statusMeasureBuilder.tag("hostname" + i, hostnames.get(i));
			}
			final Measure statusMeasure = statusMeasureBuilder.build();
			putInfosToInflux(statusMeasure);

			//test which better
			for (int i = 0; i < hostnames.size(); i++) {
				final Measure ethhostMeasure = Measure.builder(measureName + ".detailed")
						.time(Instant.now())
						.addField("ethName", ethName)
						.addField("ethId", ethId)
						.addField("ethMode", ethMode)
						.addField("hostname", hostnames.get(i))
						.addField("macs", macs.get(i))
						.addField("rx_good_bytes", rx_good_bytes)
						.addField("rx_bytes_rate", rx_bytes_rate)
						.addField("tx_bytes", tx_bytes)
						.addField("tx_bytes_rate", tx_bytes_rate)
						.tag("ethName", ethName)
						.tag("ethMode", ethMode)
						.tag("hostname", hostnames.get(i))
						.build();
				putInfosToInflux(ethhostMeasure);
			}
		}
	}

	private void updateWifi(final long stationId, final String stationName, final String measureName, final String sessionId) {
		...
		final JsonObject wifiResponse = callRestWS(WS_WIFI_DATA + stationId + "/stations/", sessionId, JsonObject.class);
	...
	private FreeboxInfo retrieveData(final String sessionId, final Map<String, Integer> temps, final Map<String, Integer> fans) {
		//callAuthorizePost();
		final JsonObject result = callRestWS(WS_SYSTEM_INFO, sessionId, JsonObject.class);
		...
		if (result.get("success").getAsBoolean()) {
			...
			for (final JsonElement sensor : result.getAsJsonObject("result").getAsJsonArray("sensors")) {
				...
				temps.put(sensorId, sensor.getAsJsonObject().get("value").getAsInt());
			...
			for (final JsonElement fan : result.getAsJsonObject("result").getAsJsonArray("fans")) {
				...
				fans.put(fanId, fan.getAsJsonObject().get("value").getAsInt());

			}
		} else {
			newFreeboxInfo.setSuccess(false);
		}
		final JsonObject resultBw = callRestWS(WS_SYSTEM_DATA, sessionId, JsonObject.class);
	...
	private void putInfosToInflux(final FreeboxInfo freeboxInfo, final Map<String, Integer> temps, final Map<String, Integer> fans, final String measureName) {
		...
		final MeasureBuilder freeboxInfoMeasureBuilder = Measure.builder(measureName)
				.time(Instant.now())
				.addField("nowTime", currentMinute)
				.addField("uptimeVal", freeboxInfo.getUptimeSecond())
				.addField("lastReboot", lastRebootMinute);
		for (final Entry<String, Integer> temp : temps.entrySet()) {
			freeboxInfoMeasureBuilder.addField(temp.getKey(), temp.getValue());
		}
		for (final Entry<String, Integer> fan : fans.entrySet()) {
			freeboxInfoMeasureBuilder.addField(fan.getKey(), fan.getValue());
		}
		freeboxInfoMeasureBuilder
				...
				.tag("lastRebootHour", "" + lastRebootMinute / 60)
				.tag("reboot", lastReboot != lastRebootMinute
						&& freeboxInfo.getUptimeSecond() < 60 ? "REBOOT" : "RUN");

		final Measure freeboxInfoMeasure = freeboxInfoMeasureBuilder.build();
		lastReboot = lastRebootMinute;
	...
	private String callLoginPost() {
		...
		//not used ? final String password_salt = loginChallenge.getAsJsonObject("result").get("password_salt").getAsString();
